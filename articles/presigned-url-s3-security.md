...
今回の設計は「アップロードする」だけでなく「アップロードされたデータを公開するまで」を範囲としています。その判断として、アップロード完了と同時にデータを公開する楽観的な処理は採用しませんでした。
理由は、Content-Typeの申告値はブラウザ側で偽装できるため、申告値を頼りにした検証だけでは悪意あるファイルを確実に弾けないからです。そこでS3へのアップロード後にLambdaが非同期でファイルの実体を検証し、その結果を受けて初めてデータが公開されるフローを採用しています。これは、S3へのアップロードからLambdaによる実体検証までのラグの間にウイルスを含んだファイルが公開されることを防ぐことを意図しています。
...
![三者の役割分担](/images/upload_event_driven_notify.png)
