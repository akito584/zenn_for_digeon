...
今回の実装ではただアップロードをする処理だけではなく、実運用するにあたって十分なセキュリティを保証できる実装を心がけていたため、AIとの壁打ちの結果、ブラウザ・App Server・S3に加えてLambdaも設計に加えました。（理由は後のセキュリティ面で後述します）
しかし、このAIとの壁打ちの中でブラウザ・App Server・S3・Lambdaはそれぞれ何の役割を担っているのかごちゃごちゃしてしまったのです。ブラウザはHTTPリクエストとアップロード処理のロジックを担っているのはなんとなくわかるのですが、この際App Serverは何しているんだろう？
...
よって、「どのレベルのセキュリティが必要で、どの技術を用いるべきなのか」というのは実装を始める前に確定させるべき重要事項の一つだと実感しました。
:::
